    ...
    <article className="relative z-1 -mt-px border-x border-line bg-card max-md:border-x-0">
      ...
      <div className="sticky top-14 z-30 border-b border-line bg-card/95 backdrop-blur supports-backdrop-filter:bg-card/75">
        <div className="flex h-12 items-center justify-between gap-3 px-4 md:px-8">
          ...
          <span className="min-w-0 truncate font-ibm-plex-mono text-xs tabular-nums text-muted-foreground">
      ...
      <section className="relative grid gap-px border border-line bg-line sm:grid-cols-3">
        <MetaCard
          ...
          {l(project.collaboration.contributions, project.collaboration.contributionsId).map(
            (item, i) => (
              <li key={i} className="flex items-baseline gap-3 text-sm leading-7">
                <span className="shrink-0 font-ibm-plex-mono text-xs tabular-nums text-muted-foreground">
...
function Section({
  ...
  return (
    <section className="border-b border-line px-4 py-8 md:px-8">
      <h2 className="mb-4 text-xs font-ibm-plex-mono tracking-[0.2em] text-muted-foreground uppercase">
        {title}
      </h2>
      {children}
    </section>
...
function MetaCard({ label, value }: { label: string; value: string }) {
  return (
    <div className="bg-card px-4 py-5 md:px-6">
      <dt className="mb-1 font-ibm-plex-mono text-[1rem] tracking-[0.18em] text-muted-foreground uppercase">
        {label}
      </dt>
      <dd className="font-ibm-plex-mono text-xs text-foreground">{value}</dd>
    </div>
  )
}
